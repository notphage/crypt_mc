...
uintptr_t injection::manualmapper::inject(const std::vector<std::byte>& buffer, injection::executor::mode execution_mode) noexcept
{
	...
	// INITIALISE CONTEXT
	map_ctx ctx(fnvc("Main Image"), buffer);
...
bool injection::manualmapper::uninject(injection::executor::mode execution_mode) noexcept
{
	for (auto&& ctx : this->mapped_modules())
	{
		this->execution_engine() = injection::executor(execution_mode);
...
bool injection::manualmapper::map_image(map_ctx& ctx) noexcept
{
	...
	// UNMAP FROM LOCAL
	if (!native::process::current_process().unmap(ctx.local_image()))
		return false;

	return true;
}

bool injection::manualmapper::unmap_image(map_ctx& ctx) noexcept
{
	if (!this->process().unmap(ctx.remote_image()))
		return false;
...
uintptr_t injection::manualmapper::find_or_map_dependency(const std::string& image_name) noexcept
{
	...
	{
		if (module.image_name_hash() == fnvr(image_name.c_str()))
...
bool injection::manualmapper::call_entrypoint(map_ctx& ctx) noexcept
{
	...
	return this->execution_engine().handle_entry(ctx, this->process());
}

bool injection::manualmapper::call_exitpoint(map_ctx& ctx) noexcept
{
	return this->execution_engine().handle_exit(ctx, this->process());
}
