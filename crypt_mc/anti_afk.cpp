...
void c_anti_afk::on_get_time(const std::shared_ptr<c_game>&, const std::shared_ptr<c_player>& self, const std::shared_ptr<c_world>&)
{
	...
	static auto delay = util::random((int) ctx.m_settings.player_anti_afk_min_delay * 1000, (int) ctx.m_settings.player_anti_afk_max_delay * 1000);
	
	if (self->get_forward() != 0.f || self->get_strafing() != 0.f)
	{
		old = now;
		delay = util::random((int) ctx.m_settings.player_anti_afk_min_delay * 1000, (int) ctx.m_settings.player_anti_afk_max_delay * 1000);
	}
