...
void c_block_forge_18X::instantiate(jobject block_object, JNIEnv* _jni)
{
	jni = _jni;
	...
	if (!init_fields)
	{
		...
		blockfields.cls_block = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("net.minecraft.block.Block")));
		blockfields.cls_tile_entity = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("net.minecraft.tileentity.TileEntity")));
		blockfields.cls_block_pos = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("net.minecraft.util.BlockPos")));
		blockfields.cls_vec3i = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("net.minecraft.util.Vec3i")));

		blockfields.fid_block_pos_xpos = jni->GetFieldID(blockfields.cls_vec3i, xors("field_177962_a"), xors("I"));
		blockfields.fid_block_pos_ypos = jni->GetFieldID(blockfields.cls_vec3i, xors("field_177960_b"), xors("I"));
		blockfields.fid_block_pos_zpos = jni->GetFieldID(blockfields.cls_vec3i, xors("field_177961_c"), xors("I"));

		blockfields.mid_get_pos = jni->GetMethodID(blockfields.cls_tile_entity, xors("func_174877_v"), xors("()Lnet/minecraft/util/BlockPos;"));
		blockfields.mid_from_block = jni->GetStaticMethodID(blockfields.cls_block, xors("func_149682_b"), xors("(Lnet/minecraft/block/Block;)I"));

		init_fields = true;
	}
}

jint c_block_forge_18X::get_xpos()
{
...
jint c_block_forge_18X::get_ypos()
{
	auto obj_block_pos = jni->CallObjectMethod(block_instance, blockfields.mid_get_pos);
	if (!obj_block_pos)
		return 0.f;
...
jint c_block_forge_18X::get_zpos()
{
	auto obj_block_pos = jni->CallObjectMethod(block_instance, blockfields.mid_get_pos);
	if (!obj_block_pos)
		return 0.f;
...
jclass c_class_loader_forge_18X::find_class(const char* name)
{
...
void c_world_forge_18X::instantiate(jobject world_object, JNIEnv* _jni)
{
	...
	if (!init_fields)
	{
		...
		worldfields.fid_loaded_tile_entity_list = jni->GetFieldID(worldfields.world, xors("field_147482_g"), xors("Ljava/util/List;"));
...
std::vector<std::shared_ptr<c_player>> c_world_forge_18X::get_players()
{
	...
	{
		auto player_ptr = std::make_shared<c_player_forge_18X>();
...
std::vector<std::shared_ptr<c_block>> c_world_forge_18X::get_loaded_blocks()
{
	...
	{
		auto block_ptr = std::make_shared<c_block_forge_18X>();
...
jfloat c_player_forge_18X::get_forward()
{
...
jstring c_player_forge_18X::get_item_name(jobject item)
{
	return (jstring)jni->GetObjectField(item, playerfields.fid_get_item_name);
}

jboolean c_player_forge_18X::holding_item()
{
...
jboolean c_player_forge_18X::holding_fishing_rod()
{
...
jdouble c_player_forge_18X::old_origin_y()
{
	return jni->GetDoubleField(player_instance, playerfields.fid_old_yorigin);
}

jdouble c_player_forge_18X::old_origin_z()
{
...
void c_player_forge_18X::set_velocity(jdouble x, jdouble y, jdouble z)
{
	jni->CallVoidMethod(player_instance, playerfields.mid_set_velocity, x, y, z);
}

void c_player_forge_18X::set_position(jdouble x, jdouble y, jdouble z)
{
	jni->CallVoidMethod(player_instance, playerfields.mid_set_position, x, y, z);
}

void c_player_forge_18X::set_angles(jfloat yaw, jfloat pitch)
{
	jni->CallVoidMethod(player_instance, playerfields.mid_set_angles, yaw, pitch);
}

void c_player_forge_18X::set_sneaking(jboolean sneak)
{
...
void c_game_forge_18X::instantiate(JNIEnv* _jni)
{
	...
	if (!init_fields)
	{
		...
		gamefields.fid_the_player = jni->GetFieldID(gamefields.minecraft, xors("field_71439_g"), xors("Lnet/minecraft/client/entity/EntityPlayerSP;"));
		...
		gamefields.fid_render_xpos = jni->GetFieldID(gamefields.render_manager, xors("field_78725_b"), xors("D"));
		gamefields.fid_render_ypos = jni->GetFieldID(gamefields.render_manager, xors("field_78726_c"), xors("D"));
		gamefields.fid_render_zpos = jni->GetFieldID(gamefields.render_manager, xors("field_78723_d"), xors("D"));
...
jboolean c_game_forge_18X::is_in_inventory()
{
	return gamefields.in_inventory;
}

jboolean c_game_forge_18X::is_view_bobbing()
{
	return jni->GetBooleanField(gamefields.obj_settings, gamefields.fid_view_bobbing);
}

jboolean c_game_forge_18X::is_in_chat()
{
...
std::shared_ptr<c_world> c_game_forge_18X::get_world()
{
