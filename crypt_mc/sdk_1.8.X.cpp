...
void c_player_18X::instantiate(jobject player_object, JNIEnv* _jni)
{
	...
	if (!init_fields)
	{
		...
		playerfields.item_shovel_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("aaq")));
		playerfields.item_hoe_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("zv")));
		playerfields.item_pick_axe_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("aag")));
		playerfields.item_ender_pearl_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("zk")));
		playerfields.item_fishing_rod_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("zq")));
		playerfields.item_ender_eye_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("zj")));
		playerfields.item_exp_bottle_class = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("zl")));
...
jboolean c_player_18X::holding_axe()
{
...
jboolean c_player_18X::holding_shovel()
{
	return playerfields.holding_shovel;
}

jboolean c_player_18X::holding_sword()
{
...
void c_game_18X::instantiate(JNIEnv* _jni)
{
	...
	if (!init_fields)
	{
		...
		gamefields.cls_vec3 = (jclass)jni->NewGlobalRef(class_loader->find_class(xors("aui")));
		...
		gamefields.mid_screen_constructor = jni->GetMethodID(gamefields.cls_moving_object_position, xors("<init>"), xors("(Lpk;Laui;)V"));
...
void c_game_18X::set_key_bind_state(jint key_code, jboolean state)
{
...
jint c_game_18X::get_sneak_key_code()
{
...
void c_game_18X::set_object_mouse_over(jobject player, const vec3& hit_position)
{
	if (!player)
		return;
