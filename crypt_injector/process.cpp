...
std::uint32_t native::process::id_from_window(std::string_view window_class, std::string_view window_title) noexcept
{
	const auto window_handle = FindWindowA(window_class.length() > 0 ? window_class.data() : nullptr, window_title.length() > 0 ? window_title.data() : nullptr);
...
bool native::process::unmap(std::uintptr_t section) noexcept
{
	void* base_address = (void*)section;

	auto result = ntdll::NtUnmapViewOfSection(
		this->handle().unsafe_handle(),
		base_address);

	if (!NT_SUCCESS(result))
	{
		logger::log_error("NtUnmapViewOfSection failed");
		logger::log_formatted("Error code", result, true);
		return false;
	}
