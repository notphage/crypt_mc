...
void c_velocity::on_time(const std::shared_ptr<c_game>& mc, const std::shared_ptr<c_player>& self, const std::shared_ptr<c_world>& world)
{
	...
	auto delay_amount = ctx.m_settings.combat_velocity_delay ? self->get_max_hurt_time() - ctx.m_settings.combat_velocity_delay_ticks : 10;
	auto hurt_time = self->get_hurt_time();
