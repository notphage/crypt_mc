...
namespace util
{
	...
	static __forceinline void simple_save(uint32_t name, const void* setting, size_t size, const char* filename)
	{
		...
		std::string message( messageBuffer, FormatMessageA( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
															NULL, GetLastError( ), MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ), ( LPSTR ) &messageBuffer, 0, NULL ) );

		printf( "%lx %u %u", ( uintptr_t ) data, size, name );
		printf( "%s\n", message.c_str( ) );
		*/
	}
	...
	static __forceinline void simple_load(uint32_t name, void* setting, unsigned long size, const char* filename)
	{
		...
		// printf( "reading fromm %s ", filename );
		GetPrivateProfileStringA("_", std::to_string(name).c_str(), "", buffer, size * 2 + 1, filename);

		// LPSTR messageBuffer = nullptr;
		// std::string message( messageBuffer, FormatMessageA( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, GetLastError( ), MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ), ( LPSTR ) &messageBuffer, 0, NULL ) );

		// printf( "%s\n", message.c_str( ) );

		if (*buffer == 0)
		{
			// fall back to default setting instead of nulling it
			return;
		}
		for (size_t i = 0; i < size; i++)
		{
	...
	{
		const handle_t file = CreateFile(filename, access_flags, 0, 0, creation_flags, FILE_ATTRIBUTE_NORMAL, 0);
