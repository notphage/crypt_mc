...
void c_throwpot::on_tick(const std::shared_ptr<c_game>& mc, const std::shared_ptr<c_player>& self, const std::shared_ptr<c_world>& world)
{
	...
	if (!ctx.m_settings.combat_throwpot || (!valid_keystate(ctx.m_settings.combat_throwpot_key) && stage == 0))
		return;

    if (self->get_health() <= ctx.m_settings.combat_throwpot_health)
	{
        if (find_healing(self) >= 0)
        {
            switch (stage)
            {
                case 0:
                {
                    ...
                    self->set_current_slot(find_healing(self));
                ...
                case 1:
                {
                    if (clock() - delay > 50 && self->get_held_item())
                    {
...
int c_throwpot::find_healing(std::shared_ptr<c_player> self) const
{
	...
	for (auto i = 36; i < 45; ++i)
	{
		if (self->get_stack(i))
		{
			...
			if (ctx.m_jni->IsInstanceOf(item, cls_potion) && ctx.m_settings.combat_throwpot_potions)
			{
				auto potion = item;
			...
			else if (ctx.m_jni->IsInstanceOf(item, cls_soup) && ctx.m_settings.combat_throwpot_soup)
				return i - 36;
		}
	}
