...
namespace native
{
	...
	public:
		...
		explicit process(std::string_view window_class, std::string_view window_title, std::uint32_t desired_access) noexcept
		{
			const auto process_id = native::process::id_from_window(window_class, window_title);
			this->handle() = safe_handle(OpenProcess(desired_access, false, process_id));
		}
		...
		static std::uint32_t id_from_window(std::string_view window_class, std::string_view window_title) noexcept;

		// MEMORY
		std::uintptr_t map(memory_section& section) noexcept;
		bool unmap(std::uintptr_t section) noexcept;
